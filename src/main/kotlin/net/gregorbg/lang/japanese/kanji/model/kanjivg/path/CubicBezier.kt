...
data class CubicBezier(
    ...
    override fun toSvg(): String {
        return """
            M ${this.start.toSvg()}
            C ${this.controlStart.toSvg()} ${this.controlEnd.toSvg()} ${this.end.toSvg()}
        """.trimIndent()
    }
    ...
    companion object {
        ...
        fun fromRelative(
            start: GeomPoint,
            controlStartRelative: GeomPoint,
            controlEndRelative: GeomPoint,
            endRelative: GeomPoint,
        ): CubicBezier {
            return CubicBezier(
                start,
                start + controlStartRelative,
                start + controlEndRelative,
                start + endRelative,
            )
        }
    }
